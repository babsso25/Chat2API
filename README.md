...
## ⬇️ How to Download and Install Chat2API on Windows
...
1. Scroll down or look at the right sidebar for a section called **Releases**.
2. Click on the most recent release title (like “v1.0” or newer).
3. In the release assets, look for a file named something like `Chat2API-Setup.exe` or `Chat2API-x64.exe`. This is the installer for Windows.
...
1. After the download finishes, open the folder where the file saved.
2. Double-click the installer file to begin installation.
3. You may see a security prompt - choose **Run** or **Yes** to continue.
...
### Step 5: Launch Chat2API

Once installed:

1. A shortcut will appear on your desktop or in your Start Menu.
2. Click it to open Chat2API.
3. The app will open with a simple interface to select or add AI services.
...
1. Inside Chat2API, find the **API Endpoint** address. It will look like `http://localhost:xxxx`.
2. Open your OpenAI-compatible client software (like a chat app).
3. Enter the API Endpoint from Chat2API in the client’s API settings instead of the regular OpenAI URL.
4. Start chatting. Your messages will go through Chat2API to the chosen AI service.
